  ...
  domains.forEach((d, idx) => {
    ...
    html += '<div class="domain-row">' +
      ...
      '<div class="domain-actions" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">' +
        ...
        '<button class="btn-icon" onclick="openEditDomainModal('+idx+')" title="Edit">Edit</button>' +
        '<button class="btn-icon" onclick="deleteDomain(\''+escapeHtml(d.name || d.Name || '')+'\')" title="Delete">Delete</button>' +
      '</div>' +
    '</div>';
  });
...
function openAddDomainModal() {
  ...
  list.innerHTML = '<div class="flex gap-8 items-center"><input class="input font-mono" type="text" placeholder="http://127.0.0.1:8080" style="flex:1" required><input class="input font-mono" type="number" placeholder="Weight" value="1" min="1" style="width:80px"><button type="button" class="btn-icon" onclick="this.parentElement.remove()" title="Remove">X</button></div>';
...
function addBackendRow() {
  ...
  row.innerHTML = '<input class="input font-mono" type="text" placeholder="http://ip:port" style="flex:1" required><input class="input font-mono" type="number" placeholder="Weight" value="1" min="1" style="width:80px"><button type="button" class="btn-icon" onclick="this.parentElement.remove()" title="Remove">X</button>';
...
function openEditDomainModal(idx) {
  ...
  (d.upstreams || []).forEach(u => {
    const row = document.createElement('div');
    row.className = 'flex gap-8 items-center';
    row.innerHTML = '<input class="input font-mono" type="text" value="'+escapeHtml(u.url)+'" style="flex:1" required><input class="input font-mono" type="number" value="'+(u.weight||1)+'" min="1" style="width:80px"><button type="button" class="btn-icon" onclick="this.parentElement.remove()" title="Remove">X</button>';
    list.appendChild(row);
  });
